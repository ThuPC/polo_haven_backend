        ...
        <div class="card custom-card">
          <div class="card custom-card">
            <div class="card-header custom-card-header text-center">
              <h1 class="mb-0">QUẢN LÝ KÍCH THƯỚC</h1>
            </div>
            <div class="card-body custom-card-body">
              <div class="mb-4">
                <label for="search" class="font-weight-bold"
                  >Tên kích thước</label
                >
                <div class="d-flex align-items-center">
                  ...
                  <button
                    ...
                    @click="goToAddKichThuoc"
                  >
                    <i class="fas fa-plus"></i> Thêm tên kích thước
                  </button>
                </div>
              </div>
            </div>
          ...
          >
            <div class="table-responsive">
              <div class="table-responsive">
                <table class="table table-hover mt-3">
                  ...
                  <tbody>
                    <tr
                      v-for="(kichThuoc, index) in kichThuocList"
                      :key="kichThuoc.id"
                      class="text-center"
                    >
                      <td>{{ (currentPage - 1) * pageSize + index + 1 }}</td>
                      <td>{{ kichThuoc.maKichThuoc }}</td>
                      <td>{{ kichThuoc.size }}</td>
                      <td>
                        <label class="switch">
                          <input
                            type="checkbox"
                            :checked="kichThuoc.trangThai === 1"
                            @change="toggleStatus(kichThuoc)"
                          />
                    ...
                    <tr v-if="!loading && kichThuocList.length === 0">
            ...
            >
              ...
              <div class="modal-custom">
                ...
                <!-- Form -->
                <form @submit.prevent="updateKichThuoc">
                  <div class="mb-3">
            ...
            >
              ...
              <div class="modal-custom">
                ...
                <!-- Form -->
                <form @submit.prevent="handleAddKichThuoc">
                  <div class="mb-3">
...
import {
  ...
  updateKichThuoc,
  deleteKichThuoc,
  changeStatusKichThuoc,
} from "@/services/RoleQuanLy/KichThuocService";
...
export default {
  ...
  methods: {
    ...
    async fetchKichThuoc() {
      ...
      try {
        ...
        if (data && Array.isArray(data.content)) {
          this.kichThuocList = data.content;
          this.totalPages = data.totalPages || 1;
        } else {
          this.kichThuocList = [];
          this.totalPages = 0;
          this.error = "Dữ liệu kích thước không hợp lệ!";
        }
      } catch (error) {
        this.kichThuocList = [];
        this.totalPages = 0;
        this.error = "Không thể tải danh sách kích thước!";
    ...
    async updateKichThuoc() {
      const tenMoi = this.editSize ? this.editSize.trim() : "";

      if (!tenMoi) {
        this.showNotification("Vui lòng nhập size!", "error");
        return;
      }

      // const sizeNumber = parseInt(tenMoi, 10);
      // if (isNaN(sizeNumber) || sizeNumber < 1 || sizeNumber > 40) {
      //   this.showNotification("Kích thước phải là số từ 1 đến 40!", "error");
      //   return;
      // }

      const isDuplicate = this.kichThuocList.some(
        (kt) =>
          kt.id !== this.selectedKichThuoc.id && // bỏ qua chính nó
          String(kt.size ?? "")
            .trim()
      ...
      if (isDuplicate) {
        this.showNotification("Kích thước đã tồn tại!", "error");
        return;
      }

      try {
        await updateKichThuoc(this.selectedKichThuoc.id, tenMoi);
        this.showNotification("Cập nhật kích thước thành công!", "success");
        this.showEditModal = false;
        this.fetchKichThuoc(); // Refresh lại list
      } catch (error) {
    ...
    editKichThuoc(item) {
      this.selectedKichThuoc = item;
      this.editSize = item.size;
      this.showEditModal = true;
    },

    async deleteKichThuocHandler(id) {
      if (confirm("Bạn có chắc chắn muốn xoá kích thước này?")) {
        try {
          await deleteKichThuoc(id);
          this.showNotification("Xoá thành công!", "success");
          this.fetchKichThuoc();
        } catch (error) {
          console.error("Lỗi xoá kích thước:", error);
          this.showNotification("Xoá thất bại!", "error");
        }
      }
    },

    async toggleStatusKichThuoc(item) {
      const newStatus = item.trangThai === 1 ? 0 : 1;
      try {
        await changeStatusKichThuoc(item.id, newStatus);
    ...
    async handleAddKichThuoc() {
      const newSizeTrimmed = this.newSize ? this.newSize.toString().trim() : "";
      ...
      // const sizeNumber = parseInt(newSizeTrimmed, 10);
      // if (isNaN(sizeNumber) || sizeNumber < 1 || sizeNumber > 40) {
      //   this.showNotification("Kích thước phải là số từ 1 đến 40!", "error");
      //   return;
      // }
      ...
      const isDuplicate = this.kichThuocList.some(
        (kt) =>
          String(kt.size ?? "")
            .trim()
            .toLowerCase() === newSizeTrimmed.toLowerCase()
      );
      ...
      try {
        const response = await createKichThuoc({ size: newSizeTrimmed });
        const newItem = response.data.data;

        this.showNotification("Thêm kích thước thành công!", "success");
        this.closeModal();

        if (this.currentPage === 1) {
          this.kichThuocList.unshift(newItem);
          if (this.kichThuocList.length > this.pageSize) {
            this.kichThuocList.pop();
          }
        } else {
          this.currentPage = 1;
          this.fetchKichThuoc();
        ...
        this.showNotification("Thêm thất bại!", "error");
      }
    },
