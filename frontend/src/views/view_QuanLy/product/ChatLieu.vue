        ...
        <div class="card custom-card">
          <div class="card custom-card">
            <div class="card-header custom-card-header text-center">
              <h1 class="mb-0">QUẢN LÝ CHẤT LIỆU</h1>
            </div>
              ...
              <div class="mb-4">
                ...
                <div class="d-flex align-items-center">
                  ...
                  <button
                    ...
                    @click="goToAddChatLieu"
                  >
                    <i class="fas fa-plus"></i> Thêm tên chất liệu
                  </button>
                </div>
              </div>
            </div>
          ...
          >
            <div class="table-responsive">
              <table class="table table-hover mt-3">
                ...
                <tbody>
                  <tr
                    v-for="(chatLieu, index) in chatLieuList"
                    :key="chatLieu.id"
                    class="text-center"
                  >
                    ...
                    <td>
                      <label class="switch">
                        <input
                          type="checkbox"
                          :checked="chatLieu.trangThai === 1"
                          @change="toggleStatus(chatLieu)"
                        />
                  ...
                  <tr v-if="!loading && chatLieuList.length === 0">
            ...
            >
              ...
              <div class="modal-custom">
                ...
                <h4 class="modal-title">Sửa Chất Liệu</h4>

                <!-- Form -->
                <form @submit.prevent="updateChatLieu">
                  <div class="mb-3">
            ...
            >
              ...
              <div class="modal-custom">
                ...
                <!-- Form -->
                <form @submit.prevent="handleAddChatLieu">
                  <div class="mb-3">
...
export default {
  ...
  methods: {
    ...
    async fetchChatLieu() {
      ...
      try {
        ...
        if (data && Array.isArray(data.content)) {
          this.chatLieuList = data.content;
          this.totalPages = data.totalPages || 1;
        } else {
          this.chatLieuList = [];
          this.totalPages = 0;
          this.error = "Dữ liệu chất liệu không hợp lệ!";
          console.error("Dữ liệu không hợp lệ:", data);
        }
      } catch (error) {
        this.chatLieuList = [];
        this.totalPages = 0;
        this.error = "Không thể tải danh sách chất liệu!";
    ...
    async updateChatLieu() {
      const tenMoi = this.editChatLieuName ? this.editChatLieuName.trim() : "";

      if (!tenMoi) {
        this.showNotification("Vui lòng nhập tên chất liệu!", "error");
        return;
      }

      // Kiểm tra trùng (bỏ qua chính chất liệu đang chỉnh sửa)
      const isDuplicate = this.chatLieuList.some(
        (cl) =>
          cl.id !== this.selectedChatLieu.id &&
          String(cl.tenChatLieu ?? "")
            .trim()
            .toLowerCase() === tenMoi.toLowerCase()
      );

      if (isDuplicate) {
        this.showNotification("Chất liệu đã tồn tại!", "error");
        return;
      }

      try {
        await updateChatLieu(this.selectedChatLieu.id, tenMoi);
        this.showNotification("Cập nhật chất liệu thành công!", "success");
        this.showEditModal = false;
        this.fetchChatLieu();
      } catch (error) {
        console.error("Lỗi khi cập nhật chất liệu:", error);
        this.showNotification("Cập nhật chất liệu thất bại!", "error");
      }
    },

    editChatLieu(item) {
      this.selectedChatLieu = item;
      this.editChatLieuName = item.tenChatLieu;
      this.showEditModal = true;
    },

    async deleteChatLieu(id) {
      if (confirm("Bạn có chắc chắn muốn xoá chất liệu này?")) {
        try {
          await deleteChatLieu(id); // Gọi hàm API
          this.showNotification("Xoá chất liệu thành công!", "success");
          this.fetchChatLieu(); // Refresh danh sách chất liệu
        } catch (error) {
          console.error("Lỗi xoá chất liệu:", error);
          this.showNotification("Xoá chất liệu thất bại!", "error");
        }
      }
    },
    ...
    async toggleStatus(item) {
      ...
      try {
        await changeStatusChatLieu(item.id, newStatus);
    ...
    async handleAddChatLieu() {
      const tenMoi = this.newChatLieuName ? this.newChatLieuName.trim() : "";
      ...
      try {
        const response = await createChatLieu({
          tenChatLieu: tenMoi,
        });

        const newItem = response.data.data;

        this.showNotification("Thêm chất liệu thành công!", "success");
        this.closeModal();

        if (this.currentPage === 1) {
          this.chatLieuList.unshift(newItem);
          if (this.chatLieuList.length > this.pageSize) {
            this.chatLieuList.pop();
          }
        } else {
          this.currentPage = 1;
          this.fetchChatLieu();
        ...
        this.showNotification("Thêm chất liệu thất bại!", "error");
      }
    },
  ...
  watch: {
    searchKeyword() {
      ...
      this.searchDebounce = setTimeout(() => {
        this.currentPage = 1;
        this.fetchChatLieu(); // hoặc fetchMauSac(), fetchChatLieu()
      }, 500);
    },
  },
